...
def addExtras(quintuplets: list, leftoverStudents: list, matchBefore: dict):
    for student in leftoverStudents:
        ...
        for i in range(len(quintuplets)):
            ...
            if isValidGroup(matchBefore, tempList):
        ...
        if index == -1:
            print("Error: literally could not find student a group to match to")
        else:
...
def makeQuintuplets(quads: list, aloneStudents: dict, singles: dict, matchBefore: dict):
    ...
    for indexRun in range(5):
        print("Length extra students: "+str(len(extraStudentsAlone)))
...
def makePairs(students: dict, matchDict: dict):
        ...
        for index1 in range(len(extraStudents)):
            for index2 in range(len(extraStudents)):
                if index1 != index2 and (index1 not in listUsedIndexes) and (index2 not in listUsedIndexes):
                    if isValidGroup(matchDict, [extraStudents[index1], extraStudents[index2]]):
    ...
    if len(extraStudents)%2 == 1:
        print("odd number of extra students in pairs, problem")
...
def makeQuads(students: dict, matchDict: dict, pairs: list):
    ...
    for index in range(5):
        print("Length extra students: "+str(len(extraStudents)))
        listUsedIndexes = list()
        pairsToMake = list()
        for index1 in range(len(extraStudents)):
            for index2 in range(len(extraStudents)):
                if index1 != index2 and (index1 not in listUsedIndexes) and (index2 not in listUsedIndexes):
                    if isValidGroup(matchDict, [extraStudents[index1][0], extraStudents[index1][1], extraStudents[index2][0], extraStudents[index2][1]]):
                        listUsedIndexes.append(index1)
                        listUsedIndexes.append(index2)
                        pairsToMake.append([index1, index2])
...
def findMinIndex(quads: list, matchedBefore: dict):
    index = 0
    ...
    for i in range(len(quads)):
        ...
        if scoreTwoByTwo(pair1, pair2, matchedBefore) < minScore:
            minScore = scoreTwoByTwo(pair1, pair2, matchedBefore)
            index = i
...
def cleanQuads(quads: list, singles: dict, numGroups: int, matchedBefore: dict):
    ...
    while(len(quads) > numGroups):
        index = findMinIndex(quads, matchedBefore)
...
"""
    ...
    #pairs1 = makePairs(singles, matchBefore, singles1, "OneByOne")
    #pairs2 = makePairs(singles, matchBefore, singles2, "OneByOne")
    pairs = list()
    ...
    quads = makePairs(singles, matchBefore, pairs, "TwoByTwo")
    #quads = makeQuads(singles, matchBefore, pairs)

    #The quads needed to be broken down if there are too many
    cleanQuads(quads, extraStudents, numGroups, matchBefore)

    leftoverStudents = list()
    #I need a list of aloneStudents that is equal to the size of numGroups
    while(len(extraStudents) != numGroups):
        if len(extraStudents) > numGroups:
            leftoverStudents.append(((extraStudents.popitem())[1]))
        elif len(extraStudents) < numGroups:
    ...
    quintuplets = makeQuintuplets(quads, extraStudents, singles, matchBefore)

    if len(leftoverStudents) > 0:
        groups =  addExtras(quintuplets, leftoverStudents, matchBefore)
